...
function ultra_theme_settings(){
	...
	if ( ! class_exists( 'MetaSliderPlugin' ) && ! class_exists( 'SmartSlider3' ) ) {
		$description = sprintf(
			__( 'This theme supports <a href="%s" target="_blank">Smart Slider 3</a>. <a href="%s">Install it</a> for free to create beautiful responsive sliders - <a href="%s" target="_blank">More Info</a>', 'ultra' ),
			'https://purothemes.com/smart-slider-3/',
			ultra_smartslider_install_link(),
			'https://purothemes.com/documentation/ultra-theme/home-page-slider/'
		);
	}
	
	$settings->add_field( 'home', 'slider', 'select', __('Home Page Slider', 'ultra'), array(
		'options' => ultra_sliders_get_options( true ),
		'description' => $description,
	));
	
	$settings->add_field( 'home', 'slider_stretch', 'checkbox', __( 'Stretch Home Slider', 'ultra' ), array(
		'label'       => __( 'Stretch', 'ultra' ),
		'description' => __( 'Stretch the home page slider to the width of the screen if using the full width layout.', 'ultra' ),
	) );
	...
	$settings->add_field('blog', 'related_posts', 'checkbox', __('Related Posts', 'ultra'), array(
		'description' => __('Display related posts on the single post page.', 'ultra')
	));		
	...
	$settings->add_teaser('comments', 'ajax_comments', 'checkbox', __('AJAX Comments', 'ultra'), array(
		'description' => __('Allow users to submit comments without a page re-load.', 'ultra'),
	));	 	
